...
// Done: Remove this and use numeric_limits directly whenever this bug is fixed.
// Jas: This has been patched since instead of Windows 10 SDK 10.0.10586.0 and 2015-2017 142V build tools. 
//It uses the patched Windows 10 SDK 10.0.17763.0 and 2015-2019 142V build tools. 
//Since thoses are the ones the later Slippie build with the Rust extension uses and carries the signaling NAN fix.
template <>
constexpr double SNANConstant()
{
	return std::numeric_limits<double>::signaling_NaN(); //(_CSTD _Snan._Double);
}
template <>
